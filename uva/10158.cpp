...
void dfs(ll u,ll x){
    ...
    for(ll i=0;i<sz;i++){
        ll v=graph[u][i];
        ll c=cost[u][i]; //cout<<"in dfs:"<<u<<" "<<v<<" "<<c<<"\n";
        if(vis[v]==0){
            dfs(v,x^c);
        }
    }
...
{ //freopen("10158 output","w",stdout);
    ...
    while(scanf("%lld",&n)==1){
        ...
        while(1){
            ...
            inp[k][3]=v;
            if(c==1 || c==2){
                ll x=find_par(u); //cout<<"par of:"<<u<<" is:"<<x<<"\n";
                ll y=find_par(v); //cout<<"par of:"<<v<<" is:"<<y<<"\n";
                if(x!=y){
                    joint(x,y);
                    if(c==1){
                        graph[u].push_back(v);
        ...
        for(i=0;i<n;i++){
            if(vis[i]==0){ //cout<<"dfs of:"<<i<<"\n";
                cum[i]=0;
                dfs(i,0);
            }
        }
//        cout<<"cumulative sum\n";
//        for(i=0;i<n;i++) cout<<i<<" "<<cum[i]<<"\n";

        for(i=1;i<=k;i++){
            if(ans[i]==3){ //cout<<"wtf:"<<i<<"\n";
                c=inp[i][1];
                u=inp[i][2];
                v=inp[i][3]; //cout<<"checking:"<<c<<" "<<u<<" "<<v<<"\n";
                //cout<<"cum[u]:"<<cum[u]<<" cum[v]:"<<cum[v]<<"\n";
                //ll x=find_par(u);
                //ll y=find_par(v);
        ...
        for(i=1;i<=k;i++){ //cout<<"i:"<<i<<"\n";
            if(ans[i]!=2){ //cout<<inp[i][1]<<" "<<inp[i][2]<<" "<<inp[i][3]<<" ";
//                cout<<ans[i]<<"\n";
                printf("%lld\n",ans[i]);
            }
        }
