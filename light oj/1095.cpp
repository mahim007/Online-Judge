...
ll fun(ll n,ll r){
    ...
    res=((fun(n-1,r)%mod)+(fun(n-1,r-1)%mod))%mod;
    return res;
}
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        ll p1=fun(m,q);
        ll p2=fact[n-q]; //cout<<"p1:"<<p1<<" p2:"<<p2<<"\n";

        for(i=1;i<=(m-q);i++){
            if(i&1LL) p2-=((fun(m-q,i)%mod) * (fact[n-q-i])%mod) %mod;
            else p2+=((fun(m-q,i)%mod) * (fact[n-q-i])%mod) %mod;
            p2=(p2+mod)%mod;
        } //cout<<"now p2="<<p2<<"\n";

        cout<<"Case "<<t<<": "<<((p1%mod)*(p2%mod))%mod<<"\n";
    }
