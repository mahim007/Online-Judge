...
ll fun(ll pos,ll L,ll R,ll moov){
    if(pos==n && L==m-1 && R==m && moov==0) return 0;

    ll &res=dp[pos][L][R][moov];
    if(res!=-1) return res;
    res=0;

    if(moov==0){
        if(L+1<R) res=max(res,fun(pos,L+1,R,0)+a[pos][L+1]);
        res=max(res,fun(pos,L,R,1));
    }
    else if(moov==1){
        if(R+1<=m) res=max(res,fun(pos,L,R+1,1)+a[pos][R+1]);
        if(L<R) res=max(res,fun(pos,L,R,2));
    }
    else{
        if(pos+1<=n) res=max(res,fun(pos+1,L,R,0)+a[pos+1][L]+a[pos+1][R]);
    }
