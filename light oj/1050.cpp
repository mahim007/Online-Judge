...
double fun(ll p,ll r,ll b){//printf("p=%lld r=%lld b=%lld\n",p,r,b);
    if(r==0 && b>=1) return 1.0;
    if(b==0 && r!=0) return 0.0;

    double &res=dp[p][r][b];
    if(res>=-1.0) return dp[p][r][b];

    if(p==0){
        res=0.0;
        if(r>0)
        res+=(1.0/(r+b))*fun(p^1,r-1,b)*r;

        if(b>0)
        res+=(1.0/(r+b))*fun(p^1,r,b-1)*b;

        return res;
    }
    else{
        res=0.0;
        if(b>0) res=fun(p^1,r,b-1);
        return res;
    }
}
