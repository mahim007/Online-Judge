...
double fun(ll pos,ll m){ //cout<<"pos:"<<pos<<" m:"<<m<<"\n";
    if(pos==0){
        if(m<=0){
            return 0.0;
        }
    ...
    double &res=dp[pos][m];
    if(vis[pos][m]==now) return res;
    vis[pos][m]=now;
    res=0;

    res=fun(pos-1,m);
    res=min(res,prob[pos]+(1.0-prob[pos])*fun(pos-1,m-cost[pos]));
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        while(lo<=hi){ //cout<<"lo:"<<lo<<" hi:"<<hi;
            ll mid=(lo+hi)/2; //cout<<" mid:"<<mid<<"\n";
            now++;
            double pp=fun(n,mid); //cout<<"pp:"<<pp<<"\n";
            if(pp<p){ //cout<<"true...\n";
                lo=mid+1;
                best=max(best,mid);
            }
