...
vector<ll>G[mxn],rev_G[mxn];

void dfs(ll u){ //printf("now at:%lld\n",u);
    vis[u]=1;
    ans[u]=1;
...
void bellmenford(ll n,ll m){
    ...
    for(ll j=0;j<m;j++){
        ...
        if(dis[u]+z<dis[v]){ //printf("u=%lld v=%lld z=%lld\n",u,v,z);
            f=1;
            dis[v]=dis[u]+z;
            dfs(v);
        }
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        if(f==0) printf(" impossible\n");
            ...
            for(i=0;i<n;i++){
                if(ans[i]==1) printf(" %lld",i);
            }
            printf("\n");
        }

        for(i=0;i<=n;i++) rev_G[i].clear();
    }
