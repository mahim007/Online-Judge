...
ll fun(ll pos,ll c,ll v){
    ...
    if(s[pos]=='?'){
        ll p1=(c>=4)?1: fun(pos-1,c+1,0);
        ll p2=(v>=2)?1: fun(pos-1,0,v+1);
        return res=p1|p2;
    }

    if(is_vowel(s[pos])){
        res=(v>=2)?1: fun(pos-1,0,v+1);
        return res;
    }
    else{
        return res=(c>=4)?1:fun(pos-1,c+1,0);
    }
}
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        gets(s);
        mem(dp,-1);
        ll ans=fun(strlen(s)-1,0,0);
        printf("Case %lld: ",t);

        if(ans==3) printf("MIXED\n");
