...
ll calc(ll pos,ll tight,ll extra){
    if(pos>(sz-1)/2) return 1;

    ll &res=dp[pos][tight][extra];
    if(res!=-1) return res;
    res=0;

    ll i=(pos==0)?1:0;
    for(;i<=9;i++){
        res+=calc(pos+1,tight,extra);
    }

    return res;
}

ll fun(ll pos,ll tight,ll extra){
    if(pos>(sz-1)/2){
        if(tight==1) return !extra;
        return 1;
    }
    ...
    ll i=(pos==0)?1:0;
    ll x=(tight==1)?V[pos]:9;

    if(tight==0){
        res=10*fun(pos+1,0,0);
    }
    else{
        for(;i<x;i++){
            res+=fun(pos+1,0,0);
        ...
        else if(i==V[sz-pos-1]){
            res+=fun(pos+1,1,extra);
        }
        else res+=fun(pos+1,1,0);
    }

    return res;
}

ll solve(ll n){ //cout<<"solving:"<<n<<"\n";
    if(n<0) return 0;
    if(n<10) return n+1;
    while(n){
        V.push_back(n%10);
    ...
    ll ans=10;
    for(ll i=2;i<V.size();i++){ //cout<<"for i="<<i<<" ";
        sz=i;
        mem(dp,-1);
        ll d=calc(0,0,0); //cout<<"d:"<<d<<"\n";
        ans+=d;
    }
