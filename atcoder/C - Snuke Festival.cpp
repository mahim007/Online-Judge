...
int main()
{
    ...
    for(i=1;i<=n;i++){
        ll x=a[2][i]; //cout<<"now x="<<x+1<<" pos=";
        ll idx=lower_bound(a[3]+1,a[3]+1+n,x+1)-(a[3]); //cout<<idx<<"\n";
        sum[2][i]=sum[3][n]-sum[3][idx-1];
    }
    ...
    for(i=1;i<=n;i++){
        ll x=a[1][i];
        ll idx=lower_bound(a[2]+1,a[2]+1+n,x+1)-(a[2]);
        sum[1][i]=sum[2][n]-sum[2][idx-1];
    }
