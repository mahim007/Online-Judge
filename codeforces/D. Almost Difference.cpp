...
void init(ll n,ll st,ll ed){ //printf("n=%lld st=%lld ed=%lld\n",n,st,ed);
    if(st==ed){
        tree[n].V.push_back(a[st]);
        return;
    }
    ...
    init(n*2,st,mid);
    init((n*2)+1,mid+1,ed);
    tree[n].V.insert(tree[n].V.end(),tree[2*n].V.begin(),tree[2*n].V.end());
    tree[n].V.insert(tree[n].V.end(),tree[(2*n)+1].V.begin(),tree[(2*n)+1].V.end());
    //merge (tree[2*n].V.begin(),tree[2*n].V.end(),tree[(2*n)+1].V.begin(),tree[(2*n)+1].V.end(),tree[n].V.begin());
}

ll query(ll n,ll st,ll ed,ll x,ll y,ll val){
    if(st>y || ed<x) return 0;
    if(st>=x && ed<=y) return count(tree[n].V.begin(),tree[n].V.end(),val);

    ll mid=(st+ed)/2;
    ll p1=query(2*n,st,mid,x,y,val);
    ll p2=query((2*n)+1,mid+1,ed,x,y,val);
    return p1+p2;
}
