...
ll fun(ll pos,ll taken,ll pre){ //printf("pos=%lld taken=%lld pre=%lld\n",pos,taken,pre);
    if(taken>y) return inf;
    if(pos<0LL){
        if(taken>=x && taken<=y) return 0LL;
        else return inf;
    }

    if(dp[pos][taken][pre]!=-1) return dp[pos][taken][pre];
    ll p1=0;
    ll p2=0;

    if(pre==2){
        p1=val[0][pos]+fun(pos-1,taken+1,0LL);
        p2=val[1][pos]+fun(pos-1,taken+1,1LL);
    }
    else{
        if(taken<x){
            p1=val[pre][pos]+fun(pos-1,taken+1,pre);
            p2=inf;
        }
        else if(taken>=x && taken<y){
            p1=val[pre][pos]+fun(pos-1,taken+1,pre);
            p2=val[pre^1][pos]+fun(pos-1,1,pre^1);
        }
        else{
            p1=val[pre^1][pos]+fun(pos-1,1,pre^1);
            p2=inf;
        }
    }

    return dp[pos][taken][pre]=min(p1,p2);
}
