...
int main()
{
    ...
    for(i=1;i<=3;i++) cin>>a[i][1]>>a[i][2]>>a[i][3];
    for(i=1;i<=3;i++) cin>>b[i][1]>>b[i][2]>>b[i][3];
    ...
    ll xx=-1,yy=-1;
    ll cycle=0;
    for(i=1;i<=n;i++){ //printf("loop m=%lld q=%lld\n",m,q);
        if(M[m][q]>0){
            xx=m;
            yy=q;
            cycle=1; //printf("breaking m=%lld q=%lld\n",m,q);
            break;
        }
     ...
     //printf("cycle xx=%lld yy=%lld\n",xx,yy);

    //for(i=0;i<V.size();i++) printf("%lld %lld\n",V[i].first,V[i].second);

    if(cycle==0){ //cout<<"here\n";
        cout<<al<<" "<<bob<<"\n";
    }
    else{
        al=bob=0;
        ...
        for(i=0;i<V.size();i++){ //printf("i=%lld ",i);
            ll pp=V[i].first;
            ll qq=V[i].second; //printf("pp=%lld qq=%lld\n",pp,qq);
            if(found==-1){
                if(pp==xx && qq==yy){
                    found=i;
                    len=1;
                    tmpal+=var[xx][yy].al;
                    tmpbob+=var[xx][yy].bob;
                }
                else{
                    ll x=V[i].first;
                    ll y=V[i].second;
                ...
                len++;
                tmpal+=var[pp][qq].al;
                tmpbob+=var[pp][qq].bob;
            }
        } //printf("len=%lld al=%lld bob=%lld\n",len,tmpal,tmpbob);
