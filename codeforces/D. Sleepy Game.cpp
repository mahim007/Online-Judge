...
ll vis[mxn][2],vs[mxn];
ll in[mxn],src,win,cycle;
stack<ll>S;
vector<ll>G[mxn],RG[mxn];
vector<ll>Leaf;
...
void dfs(ll u,ll col){ //cout<<"u="<<u<<" col="<<col<<"\n";
    vis[u][col]=1;
    S.push(u);
...
int main()
{
    ...
    if(Leaf.size()>0){
        for(i=0;win==0 && i<Leaf.size();i++){
            if(vis[Leaf[i]][0]==0){
                dfs(Leaf[i],0);
            }
        }  //cout<<win<<" "<<vis[src][0]<<" "<<cycle<<"\n";

//        if(win==0 && vis[src][0]!=0 && cycle==0){
//            cout<<"Lose\n";
//        }
//        else cout<<"Draw\n";
    }
