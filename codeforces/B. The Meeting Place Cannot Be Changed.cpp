...
double fun(double mid){
    double res=0.0;
    for(ll i=1;i<=n;i++){
        res=max(res,fabs((point[i]*1.0)-mid)/(speed[i]*1.0));
    }
...
int main()
{
    ...
    while(k<65){ //printf("lo=%f hi=%f\n",lo,hi);
        double mid=(lo+hi)/2.0;
        double res1=0.0;
        double res2=0.0;

        double mid1=(lo+mid)/2.0;
        double mid2=(mid+hi)/2.0;

        res1=fun(mid1); //printf("res1=%f res2=%f\n",res1,res2);
        res2=fun(mid2); //printf("res1=%f res2=%f\n\n",res1,res2);
