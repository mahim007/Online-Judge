...
ll fun(ll ps,ll q,ll tight){ //cout<<ps<<" "<<q<<" "<<tight<<"\n";
    if(ps>=s.size()){
        if(q==0) return 0;
        if((go[q]+1)==m) return 1;
        else return 0;
    }
    if(q>1000) return 0;

    ll &res=dp[ps][q][tight];
    if(res!=-1) return res;
    res=0;

    if(tight==1){
        if(s[ps]=='1'){
            res+=fun(ps+1,q+1,tight)%mod;
            res+=fun(ps+1,q,0)%mod;
        }
        else{
            res+=fun(ps+1,q,tight)%mod;
        }
    }
    else{
        res+=fun(ps+1,q+1,0)%mod;
        res+=fun(ps+1,q,0)%mod;
    }
...
int main()
{
    ...
    mem(dp,-1);
    calc();
    //for(i=1;i<=15;i++) cout<<i<<" step="<<go[i]<<"\n";
    ll ans=(fun(0,0,1))%mod;
    cout<<ans<<"\n";
