...
void init(ll idx,ll l,ll r){
    ...
    init(L,l,mid);
    init(R,mid+1,r);
    tree[idx]=__gcd(tree[L],tree[R]);
}

void query(ll idx,ll l,ll r,ll x,ll y,ll val){
    if(l>y || r<x) return;
    if(l>=x && r<=y && tree[idx]%val==0) return;
    if(l==r){
        if(tree[idx]%val){
            cnt++;
        }
    ...
    query(L,l,mid,x,y,val);
    query(R,mid+1,r,x,y,val);
}

void update(ll idx,ll l,ll r,ll x,ll val){
    if(l>x || r<x) return;
    if(l==x && r==x){ //cout<<"l="<<l<<" r="<<r<<" idx="<<idx<<"\n";
        tree[idx]=val; //cout<<"update complete!\n";
        return;
    }
    ...
    update(L,l,mid,x,val);
    update(R,mid+1,r,x,val);
