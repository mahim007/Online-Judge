...
int main()
{
    ...
    while(k--){
        M[x]=1;
        V.push_back(x); //cout<<x<<" ";
        x=x*q;
        if(q==-1 || q==1) break;
    } //cout<<"power calculation..\n";
    //cout<<"max range:"<<V.size()<<" max data:"<<V[V.size()-1]<<"\n";

    if(q==-1) V.push_back(-1LL);
    freq[0]=1;
    ...
    for(i=1;i<=n;i++){
        ...
        freq[cum[i]]++;
        //cout<<"now:"<<cum[i]<<"\n";
        ll p=V.size(); //cout<<"size of p:"<<p<<"\n";
        //p--;
        while(p>0){ //cout<<p<<"\n";
            p--;
            //if(V[p]>cum[i]) continue;
            ll num=V[p]; //cout<<"num:"<<num<<"\n";
            ll dif=cum[i]-num; //cout<<"difference:"<<dif<<"\n";
            if(freq.find(dif)!=freq.end()){
                ans+=freq[dif]; //cout<<"pow:"<<num<<" dif:"<<dif<<"\n";cout<<"ok!!!\n"; system("pause");
            }
        }
    }
