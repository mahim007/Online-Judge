...
#define mxn 100005
ll snow[mxn],temp[mxn],sum[mxn],cumu[mxn],rem[mxn];
...
int main()
{
    ...
    for(i=1;i<=n;i++){ //cout<<"i="<<i<<"\n";
        ll element=snow[i];
        ll pre=sum[i-1]; //cout<<"element="<<element<<" pre="<<pre<<"\n";
        ...
        cumu[i]+=1;
        cumu[best+1]-=1;
        rem[best+1]+=element-(sum[best]-pre);
    }
