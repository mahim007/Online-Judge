...
void insert_data(ll x){ //printf("insert %lld\n",x);
    data *cur=root;
    for(ll i=20;i>=0;i--){
        ll bit=check(x,i);
        if(cur->nxt[bit]==NULL){
            cur->nxt[bit]=new data();
        }
...
ll mex(ll x){
    ...
    for(ll i=20;i>=0;i--){
        ll bit=check(x,i);
        if(cur==NULL || cur->nxt[bit]==NULL) return ret;
        if(cur->nxt[bit]->cnt>=(1LL<<i)){
            cur=cur->nxt[bit^1LL];
            ret|=(1LL<<i);
        }
        else{
