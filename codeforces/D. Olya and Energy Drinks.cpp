...
void bfs(){
    ...
    while(!Q.empty()){ mx=max(mx,(ll)Q.size());
        data U=Q.front();
        Q.pop();
        ll ux,uy;
        ux=U.x;
        uy=U.y; //cout<<"now at u="<<ux<<" v="<<uy<<"vis[u][v]:"<<vis[ux][uy]<<"\n";

        ll i;
        for(i=1;i<=q;i++){
            if(valid(ux,uy+i)==0 || (vis[ux][uy+i]<vis[ux][uy]+1)){
                break;
            }

            if(vis[ux][uy+i]==inf)
            Q.push(data(ux,uy+i));
            vis[ux][uy+i]=vis[ux][uy]+1;
        }
        ...
        for(i=1;i<=q;i++){
            if(valid(ux+i,uy)==0 || (vis[ux+i][uy]<vis[ux][uy]+1)){
                break;
            }

            if(vis[ux+i][uy]==inf)
            Q.push(data(ux+i,uy));
            vis[ux+i][uy]=vis[ux][uy]+1;
        }
...
//        }cout<<"\n";
//    } cout<<"\n";
    if(vis[dsx][dsy]==inf) cout<<"-1\n";
    else    cout<<vis[dsx][dsy]<<"\n";  //cout<<"mx queue size:"<<mx<<"\n";
