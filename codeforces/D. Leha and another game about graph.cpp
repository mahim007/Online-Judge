...
int main()
{
    ...
    for(i=0;i<m;i++){
        ...
        if(val[u]==-1 && val[v]==-1) continue;
        if(degree[u]%2==val[u] && degree[v]%2==val[v]) continue;
        if(val[u]==-1 && (degree[v])%2==val[v]) continue;
        if(val[v]==-1 && (degree[u])%2==val[u]) continue;
        ...
        else if(val[u]==-1 && (degree[v]-1)%2==val[v]){
            ans.push_back(i+1);
            degree[u]--;
            degree[v]--;
            f=1;
        }
        else if(val[v]==-1 && (degree[u]-1)%2==val[u]){
            ans.push_back(i+1);
            degree[u]--;
            degree[v]--;
            f=1;
        }
        else if((degree[u]-1)%2==val[u] && (degree[v]-1)%2==val[v]){
            ans.push_back(i+1);
            degree[u]--;
            degree[v]--;
            f=1;
        }
        else if(f==1 && ans.size()==0){
            f=0;
        }
    }
