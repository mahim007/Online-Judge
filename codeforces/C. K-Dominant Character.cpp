...
int main()
{
    ...
    ll n,m,q;
    cin>>s+1;
    ll sz=strlen(s+1);//printf("sz=%lld\n",sz);
    for(i=1;i<=sz;i++){
        for(j=0;j<26;j++){
            a[i][j]+=a[i-1][j];
        }
...
//    for(i=1;i<=sz;i++){
//        for(j=0;j<5;j++) printf("%lld ",a[i][j]);
//        printf("\n");
//    }
    ...
    while(l<=r){ //printf("l=%lld r=%lld ",l,r);
        ll mid=(l+r)/2; //printf("mid=%lld\n",mid);
        ll occur[30];
        mem(occur,63);

        for(i=1;(i+mid-1)<=sz;i++){
            for(j=0;j<26;j++){
                occur[j]=min(occur[j],a[i+mid-1][j]-a[i-1][j]);
            }
        }

        ll x=0;
        for(i=0;i<26;i++){ //printf("%c=%lld\n",('a'+i),occur[i]);
            if(occur[i]>0) x=1;
        }
