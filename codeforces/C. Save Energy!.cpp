...
int main()
{
    ...
    double tym=(1.0*q)+(1.0*xtra); //printf("length=%0.9f\n",tym);
    //ll cycle=floor(1.0*t/gap); printf("gap=%lld cyc=%lld\n",gap,cycle);
    double on=0.0;
    double off=0.0;
    on+=1.0*(q*1.0/t);
    off+=1.0*(xtra*1.0/(2.0*t)); //printf("on=%0.9f off = %0.9f tot=%0.9f\n",on,off,on+off);
    ...
    tym=best*tym; //printf("best tym = %0.9f\n",tym);printf("best length = %0.9f\n",1.0*best*(on+off));

    double baki=1.0-(1.0*best*(on+off)); //printf("baki = %0.9f\n",baki);
