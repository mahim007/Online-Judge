...
int main()
{
    ...
    sort(jor.begin(),jor.end());
    sort(bijor.begin(),bijor.end());
    ...
    ll lastjor=m;
    if(lastjor&1) lastjor--;

    ll lastbijor=m;
    if(lastbijor%2==0) lastbijor--;

    for(i=0;i<bijor.size();i++){
        if(M[bijor[i]]>1){
            M[bijor[i]]--;

            if(bsz>jsz){ //cout<<"ekhane\n";
                bsz--;
                ll f=0;
                while(lastjor>0){
                    if(M.find(lastjor)==M.end()){
                        jsz++;
                        jor.push_back(lastjor);
                        lastjor-=2;
                        cnt++;
                        f=1;
                        break;
                ...
                ll f=0;
                while(lastbijor>0){
                    if(M.find(lastbijor)==M.end()){
                        //jsz++;
                        bijor.push_back(lastbijor);
                        lastbijor-=2;
                        cnt++;
                        f=1;
                        break;
