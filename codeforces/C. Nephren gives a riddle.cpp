...
string three="\"? Are you busy? Will you send \"";
string fifth="\"?";
...
void process(){
    ...
    ll i;
    for(i=1;i<=55;i++){
        dp[i]=1LL*one.size()+dp[i-1]+three.size()+dp[i-1]+fifth.size();
    }
...
char dfs(ll n,ll m){
    ...
    if(m<=three.size()) return three[m-1];
    m-=three.size();
    ...
    if(m<=fifth.size()) return fifth[m-1];
}
