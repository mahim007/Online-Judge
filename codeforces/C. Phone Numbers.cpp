...
int main()
{
    ...
    for(i=1;i<=q;i++){
        for(j=0;j<V[i].size();j++){
            ...
            ll tmpnum=number;
            ll nm=1;
            for(ll z=0;z<tmp.size();z++) nm*=10;

            for(k=0;k<V[i].size();k++){
                if(j==k) continue;
                if(V[i][k]=="x") continue;
                stringstream freak(V[i][k]);
                ll nwnum=0;
                freak>>nwnum;


                if(nwnum%nm==number && V[i][j].size()<=V[i][k].size()){
                    V[i][j]="x";
                }
    ...
    for(i=1;i<=q;i++){
        ...
        for(j=0;j<V[pos].size();j++){
            if(V[pos][j]!="x"){
                vv.push_back(V[pos][j]);
            }
        }

        cout<<gg<<" "<<vv.size()<<" ";
        for(j=0;j<vv.size();j++) cout<<vv[j]<<" ";
        cout<<"\n";
        vv.clear();
    }
