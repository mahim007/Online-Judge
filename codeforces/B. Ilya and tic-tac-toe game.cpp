...
ll chkclmn(ll i,ll j){
    ll cnt=0;
    for(ll k=i;k<=4;k++){
        if(grid[k][j]=='x'){
            cnt++;
        }
        else if(grid[k][j]!='x' && cnt<3){
            break;
        }
    } //cout<<"clm:"<<cnt<<"\n";
...
ll chkdig1(ll i,ll j){
    ll cnt=0;
    ll k,l;
    for(k=i,l=j;k<=4 && l<=4;k++,l++){
        if(grid[k][l]=='x'){
            cnt++;
        }
        else if(grid[k][l]!='x' && cnt<3){
            break;
        }
    } //cout<<"dig:"<<cnt<<"\n";
...
ll chkdig2(ll i,ll j){
    ll cnt=0;
    ll k,l;
    for(k=i,l=j;k<=4 && l>=1;k++,l--){
        if(grid[k][l]=='x'){
            cnt++;
        }
        else if(grid[k][l]!='x' && cnt<3){
...
ll chk(){
    for(ll i=1;i<=4;i++){
        for(ll j=1;j<=4;j++){
            if(grid[i][j]=='x'){ //cout<<"x:"<<i<<" y:"<<j<<"\n";
                if(chkclmn(i,j)) return 1;
                if(chkrow(i,j)) return 1;
                if(chkdig1(i,j)) return 1;
                if(chkdig2(i,j)) return 1;
            }
        }
    }
