    ...
    for(x=0;x<=2;x++){
        for(y=0;y<=2;y++){
            ...
            for(z=0;z<=2;z++){
                ...
                ret=min(ret,ans[i][x]+ans[j][y]+ans[k][z]);
            }
        }
    }
...
int main()
{
    ...
    cin>>n>>m;
    for(i=0;i<n;i++) cin>>s[i],ans[i][0]=ans[i][1]=ans[i][2]=inf;

    for(i=0;i<n;i++){
        for(j=0;j<m;j++){
            if(s[i][j]=='#' || s[i][j]=='*' || s[i][j]=='&'){
                ans[i][symbol]=min(ans[i][symbol],min(j,m-j));
            }
            else if(s[i][j]>='a' && s[i][j]<='z'){
                ans[i][letter]=min(ans[i][letter],min(j,m-j));
            }
            else{
                ans[i][digit]=min(ans[i][digit],min(j,m-j));
            }
