...
int main()
{
    ...
    ll x=0;
    for(i=0;i<n;i++){
        ans+=min(V[i].pro,V[i].cus);

    } //printf("ans=%lld\n",ans);

    for(i=0;i<n;i++){
        if(V[i].pro>=V[i].cus) continue;
        ll d=min(2*V[i].pro,V[i].cus)-V[i].pro; //printf("i=%lld extra=%lld\n",i,d);
        P.push_back(d);
    }
