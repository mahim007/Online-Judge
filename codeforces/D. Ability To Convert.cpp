...
int main()
{
    ...
    j=1LL;
    ll zz=1;
    for(i=s.size()-1;i>=0;i--){ //cout<<"here i="<<i<<" ans res:"<<x+((s[i]-'0')*a[j])<<"\n";
        ll p=i;
        if((x+((s[i]-'0'))*a[j])<n && a[j]!=inf && a[j]<n){
            x=x+((s[i]-'0')*a[j]);
            j++;
            //cout<<"now x="<<x<<"\n";
        }
            ...
            ans+=x*zz; //cout<<"ans:"<<ans<<"\n";
            zz=zz*n; //cout<<"z:"<<zz<<"\n";
            j=1;
            x=0;

            sz=s.size();
            for(k=i;k<sz;k++) s.pop_back();
            //x=x+((s[i]-'0'))*a[j++];
            //i++;
        }
    }

    if(x!=0){ //cout<<"at last: z="<<z<<" and x="<<x<<" and res="<<x*z[zz]<<" and="<<ans+x*z[zz]<<"\n";;
        ans+=x*zz;
    }
