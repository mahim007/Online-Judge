...
struct data{
    ll buty,cost,best1,best2;
    char ch;

    bool operator < (const data& P) const{
        if(cost==P.cost) return buty<P.buty;
        return cost<P.cost;
    }
};
...
ll fun(ll x,ll c){
    if(V[x].size()<=0) return -inf;
    //M[V[x][0].buty]++; //printf("buty=%lld\n",V[x][0].buty);
    for(ll i=1;i<V[x].size();i++){ //printf("buty=%lld\n",V[x][i].buty);
        //M[V[x][i].buty]++;

        if(V[x][i-1].best1>V[x][i].best1){
            V[x][i].best1=V[x][i-1].best1;
            V[x][i].best2=max(V[x][i].buty,V[x][i-1].best2);
        }
        else{
            V[x][i].best2=V[x][i-1].best1;
        }

    }

    ll c1=-inf;
    for(ll i=0;i<V[x].size();i++){ //printf("i=%lld ",i);
        //M[V[x][i].buty]--;
        ll rest=c-V[x][i].cost;
        if(rest<=0) continue;
        ...
        //while(V[x][bb].cost>rest) bb--;

        //printf(" cost=%lld+%lld=%lld ",V[x][i].cost,V[x][bb].cost,V[x][i].cost+V[x][bb].cost);

        if(bb<0) take=-inf;
        else if(M[V[x][bb].best1]>=1){
            take+=V[x][bb].best1;
        }
        else take+=V[x][bb].best2;

        c1=max(c1,take); //printf("c1=%lld+%lld=%lld\n",V[x][i].buty,take-V[x][i].buty,take);
        M[V[x][i].buty]++;
    }
