...
data D[mxn];
vector<ll> purple,red,blue;

ll fun2(ll x,ll y){ //printf("x=%lld y=%lld\n",x,y);
    ll c1=inf;
    ll ps1=D[x].pos;
    ll ps2=D[y].pos;
    ...
    if(bam<=dan && red[bam]>x && red[dan]<y){
        c1=min(c1,min(abs(ps1-D[red[dan]].pos), abs(ps2-D[red[bam]].pos)));

        for(ll i=bam;i<dan;i++){
            c1=min(c1,abs(abs(ps1-D[red[i]].pos)+ abs(ps2-D[red[i+1]].pos)));
        }
    }
    else c1=0;

    //printf("c1=%lld\n",c1);
    //printf("red  bam=%lld dan=%lld\n",bam,dan);
    ...
    if(bam<=dan && blue[bam]>x && blue[dan]<y){
        c2=min(c2,min(abs(ps1-D[blue[dan]].pos), abs(ps2-D[blue[bam]].pos)));

        for(ll i=bam;i<dan;i++){
            c2=min(c2,abs(abs(ps1-D[blue[i]].pos) + abs(ps2-D[blue[i+1]].pos)));
        }
    }
    else c2=0;
...
int main()
{
    ...
    for(i=1;i<=n;i++){
        scanf("%lld %c",&D[i].pos,&D[i].typ);
        if(D[i].typ=='P'){
            purple.push_back(i);
        }

        if(D[i].typ=='B') blue.push_back(i);
        if(D[i].typ=='R') red.push_back(i);
    }
    ...
    for(i=2;i<=n;i++){
        if(D[i].typ==D[i-1].typ && D[i].typ=='P'){
            ans+=abs(D[i].pos-D[i-1].pos);
        }
    }


    ///connecting left reds
    ll ps=D[purple[0]].pos;
    for(i=purple[0];i>=1;i--){
        if(D[i].typ=='P' || D[i].typ=='R'){
            ans+=abs(ps-D[i].pos);
            ps=D[i].pos;
        }
    }

    ///connecting right reds
    ps=D[purple[purple.size()-1]].pos;
    for(i=purple[purple.size()-1];i<=n;i++){
        if(D[i].typ=='P' || D[i].typ=='R'){
            ans+=abs(ps-D[i].pos);
            ps=D[i].pos;
        }
    ...
    ///connecting left blue
    ps=D[purple[0]].pos;
    for(i=purple[0];i>=1;i--){
        if(D[i].typ=='P' || D[i].typ=='B'){
            ans+=abs(ps-D[i].pos);
            ps=D[i].pos;
        }
    }

    ///connecting right blue
    ps=D[purple[purple.size()-1]].pos;
    for(i=purple[purple.size()-1];i<=n;i++){
        if(D[i].typ=='P' || D[i].typ=='B'){
            ans+=abs(ps-D[i].pos);
    ...
    ll cost1=0;
    ll cost2=0;
    for(i=0;i<purple.size()-1;i++){
        if((purple[i+1]-purple[i])==1) continue;

        cost1=inf;
        cost2=inf;
        cost2=fun2(purple[i],purple[i+1]);
        cost1=2*abs(D[purple[i]].pos-D[purple[i+1]].pos);
