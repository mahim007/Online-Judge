...
struct cmp{
    bool operator () (const pii& pp, const pii& qq){
        ll pd=f(pp.first,pp.second);
        ll qd=f(qq.first,qq.second);
        return pd>qd || (pd==qd && pp<qq);
    }
};
...
double bfs(){
    S.insert({(n+1)/2,(m+1)/2});
    vis[(n+1)/2].insert((m+1)/2);
    ll tot=0;
    ...
    while(q--){
        ...
        for(ll i=0;i<4;i++){
            ...
            if(valid(vx,vy) && vis[vx].count(vy)==0){
                S.insert({vx,vy});
                vis[vx].insert(vy);
            }
        }
