...
vector<pair<double,double> >V,ans;
...
int main()
{
    ...
    for(i=1;i<=n;i++){
        ...
        for(j=0;j<ans.size();j++){ //cout<<"dis of "<<ans[j].first<<" calculated:"<<(fabs(x-ans[j].first))<<"\n";
            if((fabs(x-ans[j].first)-(2.0*r))<=ERR){ //cout<<"taken...\n";
                V.push_back({ans[j].first,ans[j].second});
        ...
        for(j=0;j<V.size();j++){
            ...
            while(++k<55){
                ...
                double dis=sqrt(sqr(x-xx)+sqr(mid-yy));
                if((dis-2.0*r)<=ERR){
                    best=max(best,mid);
                    lo=mid+1;
                }
    ...
    for(i=0;i<ans.size();i++) cout<<ans[i].second<<" ";
    cout<<"\n";
