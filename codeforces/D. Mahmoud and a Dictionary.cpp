...
void joint(ll u,ll v){
    if(ranking[u]<ranking[v]){
        par[u]=v; //cout<<"par of u="<<u<<" is v="<<v<<"\n";
        ranking[v]+=ranking[u];
    }
    else{
        par[v]=u; //cout<<"par of v="<<v<<" is u="<<u<<"\n";
        ranking[u]+=ranking[v];
    }
}
...
//    getchar();
    ...
    for(i=1;i<=n;i++){
        ...
        ranking[i]=1;
        cum[i]=0;
        ans[i]=-1; //cout<<"string s="<<s<<" and val:"<<M[s]<<"\n";
    }

    for(i=1;i<=m;i++){ //printf(" now %lld\n",i);
        cin>>c>>p1>>p2;
//        scanLLD(c);
//        //getchar();
//        scanstr(p1);
//        scanstr(p2);
        ...
        x=findset(u); //cout<<"par of u:"<<u<<" is:"<<x<<"\n";
        y=findset(v); //cout<<"par of v:"<<v<<" is:"<<y<<"\n";

        if(x!=y){ //cout<<"joint success!!\n";
            joint(x,y);
            ans[i]=1;
            if(c==1){
                graph[u].push_back(v);
...
//    } cout<<"\n";
    ...
    for(i=1;i<=m;i++){ //cout<<"query number i:"<<i<<"\n";
        c=inp[i][1LL];
        u=inp[i][2LL];
        v=inp[i][3LL];
        if(ans[i]==1LL){
            ...
            //else pf("NO\n");
            continue;
        } //cout<<"now c="<<c<<" u="<<u<<" and v="<<v<<"\n";

        if(((cum[u]^cum[v])+1LL)==c) ans[i]=1LL;
        else ans[i]=0LL;
        //cout<<"ans found:"<<ans[i]<<"\n";
    }
