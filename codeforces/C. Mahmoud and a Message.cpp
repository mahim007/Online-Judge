...
ll fun(ll pos,ll mxx,ll mnn){ //cout<<"now:"<<pos<<" "<<taken<<"\n";
    mx=max(mx,mxx);
    if(pos==n){
        mn=min(mn,mnn); /// minimum number of substrings the message can be spit in
        return 1;
    }
    ...
    ll sz=jmp[pos];
    ll k=0;
    for(ll i=sz;i>=1 && i+pos<=n;i--){
        p+=fun(pos+i,i,mnn+1)%mod;
    }
