    ...
    for(k=1;k<=q;k++){
        ...
        if(typ==1){
            ...
            while(lo<=hi){
                ll mid1=lo+((hi-lo)/3);
                ll mid2=hi-((hi-lo)/3);
                double add1=a[mid1]*1.0+last*1.0;
                add1=add1/(mid1+1.0); //printf("mid1=%lld add1=%f\n",mid1,add1);

                double add2=a[mid2]*1.0+last*1.0;
                add2=add2/(mid2+1.0); //printf("mid2=%lld add2=%f\n",mid2,add2); system("pause");
