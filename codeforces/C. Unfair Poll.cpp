...
ll fun(ll mid){ //cout<<"mid="<<mid<<" ans="<<((min(2LL,n)*mid*m)+(m*max(n-2,0LL)*max(mid+(mid-1),0LL)))<<"\n";
    ll val=((min(2LL,n)*mid*m)+(m*max(n-2,0LL)*max(mid+(mid-1),0LL)));
    if(val<=k){
            //cout<<val<<" "<<k<<"\n";
        return 1;
    }
...
int main()
{
    ...
    while(l<=h){ //printf("low=%lld high=%lld\n",l,h);
        ll mid=(l+h)>>1;
        if(fun(mid)){
            l=mid+1;
            best=mid;
    ...
    //cout<<"calculation:"<<((min(2LL,n)*h*m)+(m*(n-2)*max(h+(h-1),0LL)))<<"\n";
    if(((min(2LL,n)*h*m)+(m*max(n-2,0LL)*max(h+(h-1),0LL)))==k){
        cout<<anshigh<<" "<<anslow<<" "<<player<<"\n";
    }
    else{ //cout<<"here k="<<k<<"\n";
        k=k-((min(2LL,n)*h*m)+(m*max(n-2,0LL)*max(h+(h-1),0LL))); //cout<<"now k="<<k<<"\n";
        ll f=0;



        for(i=max(n-1,1LL);i>=1;i--){ //cout<<i<<" ";
            for(j=1;j<=m;j++){
                k--;
                if(f==0){
                    anshigh++;
