...
int main()
{
    ...
    ll f=0;
    for(i=n-1;i>=high;i--){ //cout<<"now i="<<i<<"\n";
        ll cnt=0;
        if(i==n-1){
            lst=0;
            for(j=1;j<m+2;j++){
        ...
        if(f==1){
            lst=lp;
            ll l1=(((m+2-1)-lst)+1)+funl(i-1);
            ll r1=(lst+1)+funr(i-1); //cout<<"for lp:"<<lp<<" l:"<<l1<<" r:"<<r1<<"\n";
            l=min(l1,r1);

            lst=rp;
            ll l2=(((m+2-1)-lst)+1)+funl(i-1);
            ll r2=(lst+1)+funr(i-1);//cout<<"for rp:"<<rp<<" l:"<<l2<<" r:"<<r2<<"\n";
            r=min(l2,r2);

        }
        else{
            l=(((m+2-1)-lst)+1)+funl(i-1);
            r=(lst+1)+funr(i-1);
