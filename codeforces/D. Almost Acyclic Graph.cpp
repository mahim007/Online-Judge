...
ll vis[mxn],cycle,flag[mxn],found;
vector<ll>G[mxn],V;
vector<pair<ll,ll> >E;
map<pair<ll,ll>,ll>M;

void dfs(ll u){ //cout<<u<<" ";
...
void dfs2(ll u){ //cout<<u<<" ";
    flag[u]=1;
    for(ll i=0;i<G[u].size();i++){
        ll v=G[u][i];
        if(found==1) break;
        if(M.find({u,v})!=M.end() && M[{u,v}]==-1) continue;

        if(flag[v]==1){ //cout<<"culprit="<<v<<"\n";
            found=1;
            break;
        }
...
int main()
{
    ...
    for(i=1;i<=n;i++){ //cout<<"now i="<<i<<"\n";
        ...
        if(cycle==1){ //cout<<"cycle found...\n";
            ans++;
            if(ans>1){
                break;
            }
            for(auto it=M.begin();it!=M.end();it++){
                it->second=-1; //cout<<"removing ("<<it->first.first<<", "<<it->first.second<<")\n";
                found=0;
                for(j=0;j<V.size();j++){ //cout<<"calling from "<<V[j]<<"\n";
                    mem(flag,0);
                    dfs2(V[j]); //cout<<"\n";
                    if(found==1){
                        //cout<<"cycle found!!!\n";
    ...
    if(ans==0) cout<<"YES\n";
    else if(ans==1 && gotcha==1) cout<<"YES\n";
    else cout<<"NO\n";
