...
//    for(i=0;i<P.size();i++) cout<<P[i]<<" "; cout<<"\n";
    ...
    ll sz=V.size();
    for(i=1;i<sz;i++){
        pre[i]=pre[i-1]+M[V[i]]; //cout<<pre[i]<<" ";
    } //cout<<"\n";

//    while(r<sz){ //printf("now l=%lld r=%lld\n",l,r);
//        ll x=((V[r]/m)-((V[l]-1)/m)); //printf("x=%lld V[r]=%lld V[l]=%lld\n",x,(V[r]/m),((V[l]-1)/m));
//        if(x==q){
//            ans+=M[V[r]]*M[V[l]];
//        }
//
...
//
    ...
    for(i=1;i<sz-1;i++){//cout<<"\n\ni="<<i<<"\n";
        ll now=(V[i]/m)-((V[i]-1)/m);
        ll df=abs(q-now);
        ll lo=lower_bound(P.begin()+i,P.end(),P[i]+df)-P.begin();
        ll hi=lower_bound(P.begin()+i,P.end(),P[i]+df+1)-P.begin();
        hi--;
        if(lo>hi) lo--;
        //cout<<"lo="<<lo<<" hi="<<hi<<"\n";
        if(((V[hi]/m)-((V[i]-1)/m))==q){ //cout<<"q="<<((V[hi]/m)-((V[lo]-1)/m))<<"\n";
            ans+=(M[V[i]]*(pre[hi]-pre[lo-1])); //cout<<"x1="<<M[V[i]]<<" x2="<<(pre[hi]-pre[lo-1])<<"\n";
        }
    }
