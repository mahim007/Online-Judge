...
int main()
{
    ...
    while(!S.empty()){//printf("\n"); printf("Now size=%lld\n",sz); //system("pause");
        ...
        while(1){
            if(pos1>pos2){
                sz=sz+((lm-V[x1].begin()))-V[x1].size();
                //printf("reducing val:%d from x1:%lld\n",(((lm-V[x1].begin()))-V[x1].size()),x1);
                V[x1].erase(lm,V[x1].end());

                if(V[x1].size()==0){ //printf("x1:%lld removed\n",x1);
                    S.erase(x1);
                }
                ...
                auto min_pos=lower_bound(V[x2].begin(),V[x2].end(),pos1);
                //printf("reducing val:%d from x1:%lld\n",(((lm-V[x1].begin()))-V[x1].size()),x1);
                sz=sz+((lm-V[x1].begin()))-V[x1].size();
                V[x1].erase(lm,V[x1].end());
                ...
                x1=x2;
                lm=min_pos;
                pos1=pos2; //printf("\nnow x1=%lld lm=%lld pos1=%lld\n",x1,(ll)(lm-V[x1].begin()),pos1);

                it++;
                if(it==S.end()){//printf("come here...\n");
                    //printf("calculating: val:%d subtracted\n",(ll)(((lm-V[x1].begin()))-V[x1].size()));
                    sz=sz+((lm-V[x1].begin()))-V[x1].size();
                    V[x1].erase(lm,V[x1].end());
                    if(V[x1].size()==0){
                        S.erase(x1);
                ...
                x2=*it;
                ll ss=V[*it].size();
                pos2=V[*it][ss-1]; //printf("and x2=%lld and pos=%lld\n",x2,pos2);
            }
        }
