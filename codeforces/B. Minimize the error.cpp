...
int main()
{
    ...
    for(i=1;i<=n;i++){
        pq.push({abs(a[i]-b[i]),{a[i],b[i]}});
    }

    while(k1--){
        auto tp=pq.top();
        pq.pop();
        if(tp.second.first<=tp.second.second){
            tp.second.first++;
        }
        else{
            tp.second.first--;
        ...
        tp.first=abs(tp.second.first-tp.second.second);
        pq.push(tp);
    }
    ...
    while(k2--){
        ...
        if(tp.second.second<=tp.second.first){
            tp.second.second++;
        }
        else{
            tp.second.second--;
