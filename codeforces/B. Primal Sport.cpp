...
void sieve(){
//    flag[0]=flag[1]=1;
//    for(ll i=4;i<=x2;i=i+2){
//        flag[i]=1;
//    }

    for(ll i=2;i<=x2;i++){
        if(flag[i]==0){
            for(ll j=i+i;j<=x2;j=j+i){
                flag[j]=1;
                prime[j].push_back(i);
            }
        }
        else{
            ll sz=prime[i].size()-1;
            ll s=i-prime[i][sz]+1;
            a[i]=s;
        }
    }
...
int main()
{
    ...
    sieve();
    ll ans=inf;
    ll sz=prime[x2].size()-1; //cout<<"big prime="<<prime[x2][sz]<<"\n";
    for(ll i=x2-prime[x2][sz]+1;i<=x2;i++){ //cout<<"x0="<<x0<<"\n";
        if(flag[i]==0) continue;

        if(a[i]<ans){
            ans=a[i]; //cout<<"i="<<i<<" a[i]="<<a[i]<<"\n";
        }
    }
