...
int main()
{
    ...
    }  //for(i=0;i<fair.size();i++) printf("fair %lld=%lld\n",i,fair[i]);
    sign.push_back('&');
    sign.push_back('|');
    sign.push_back('^');
...
//    }
    ...
    if(fair.size()>=1)
    while(V[fair[0]].size()){
        cout<<sign[fair[0]]<<" "<<V[fair[0]].front()<<"\n";
        V[fair[0]].pop();
    }

    if(fair.size()>=2)
    while(V[fair[1]].size()){
        cout<<sign[fair[1]]<<" "<<V[fair[1]].front()<<"\n";
        V[fair[1]].pop();
    }

    if(fair.size()>=3)
    while(V[fair[2]].size()){
        cout<<sign[fair[2]]<<" "<<V[fair[2]].front()<<"\n";
        V[fair[2]].pop();
    }
