...
ll dfs(ll u,ll last){
    ...
    for(ll i=0;i<graph[u].size();i++){
        ...
        if(xtra>=last){
            ll val=1+dfs(v,last);
            dis[u][xtra]=max(dis[u][xtra],val);
            depth=max(depth,val);
        }
    }

    return dis[u][last]=depth;
}
...
int main()
{
    ...
    for(i=1;i<=n;i++){
        for(j=1;j<=n;j++){
                ll f=0;
            for(k=25;k>=0;k--){
                if(dis[i][k]==0 && dis[j][k]==0) continue;
                if(dis[i][k]>dis[j][k]){
                    cout<<"A";
                    f=1;
                    break;
                }
                else if(dis[i][k]<=dis[j][k]){
                    cout<<"B";
                    f=1;
                    break;
                }
