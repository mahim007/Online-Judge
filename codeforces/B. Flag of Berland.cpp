...
ll checkrow(ll r,char clr){
    for(ll i=1;i<=m;i++){
        if(grid[r][i]!=clr) return 0;
    }

    return 1;
}

ll checkcol(ll c,char clr){
    for(ll i=1;i<=n;i++){
        if(grid[i][c]!=clr) return 0;
    }

    return 1;
}

ll definerow(){//printf("in definerow()\n");
    ll x=0;
    ll i=1;
    ll j;
    while(i<=n){//printf("Now i=%lld\n",i);
        j=i;
        x=0;
        while(x<3){
            if(grid[i][1]==color[x]){
            ...
            x++;
        } //printf("selecting color: %c\n",color[x]);

        //printf("now j\n");
        while(j<=n){
            ll p=checkrow(j,color[x]); //printf("j=%lld p=%lld\n",j,p);
            if(p==0 && j==i) return 0;
            if(p==0){
                j--;
                break;
        ...
        ll span=j-i+1; //printf("span=%lld\n",span);
        i=j+1;
        if(color[x]=='R'){
            S[0].insert(span);
            if(taken[0]==0) taken[0]=1;
            else return 0;
        }
        else if(color[x]=='G'){
            S[1].insert(span);
            if(taken[1]==0) taken[1]=1;
            else return 0;
        }
        else{
            S[2].insert(span);
            if(taken[2]==0) taken[2]=1;
            else return 0;
        }
    }

    if(taken[0]==0) return 0;
    if(taken[1]==0) return 0;
    if(taken[2]==0) return 0;
    if(taken[0]!=taken[1] || taken[1]!=taken[2] || taken[0]!=taken[2]) return 0;

    if(S[0].size()!=1) return 0;
    if(S[1].size()!=1) return 0;
    if(S[2].size()!=1) return 0;

    ll num=*S[0].begin(); //printf("in definerow() num=%lld\n",num);
    if(S[0].find(num)==S[0].end()) return 0;
    if(S[1].find(num)==S[1].end()) return 0;
    if(S[2].find(num)==S[2].end()) return 0;
