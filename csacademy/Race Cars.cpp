...
ll fun1(ll id,ll pos,double tym){
    ll lo=0;
    ll hi=V.size();
    hi--;
    ll best=pos;
    while(lo<=hi){
        ll mid=(lo+hi)/2;
        if(tym-V[mid].normal>=0.0 && tym-V[mid].normal<=ERR){
            if(id<=V[mid].id){
                best=min(best,mid);
            }
            else{
...
ll fun2(ll id,ll pos,double tym){
    ll lo=0;
    ll hi=SUB.size();
    hi--;
    ll best=pos;
    while(lo<=hi){
        ll mid=(lo+hi)/2;
        if(tym-SUB[mid].normal>=0.0 && tym-SUB[mid].normal<=ERR){
            if(id<=SUB[mid].id){
                best=min(best,mid);
            }
            else{
...
int main()
{
    ...
    for(i=1;i<=n;i++){
        ...
        D.dis=len;
        D.v=spd;
        D.normal=len*1.0/spd*1.0;
        D.add=len*1.0/(spd+X*1.0);
        D.sub=len*1.0/(spd-Y*1.0);
        V.push_back(D);
...
//        }
        ...
        ll best=i;
        best=min(best,fun1(V[i].id,i,V[i].add));
        best=min(best,fun2(V[i].id,i,V[i].normal));
        ans[V[i].id]=best+1;
    }
