...
ll fun(ll r,ll c){
    ...
    if(grid[r+1][c]<grid[r+1][c+1]){
        dir[r][c]=1;
        dp[r][c]=(grid[r][c]-'a')+fun(r+1,c);
    }
    else if(grid[r+1][c]>grid[r+1][c+1]){
        dir[r][c]=2;
        dp[r][c]=(grid[r][c]-'a')+fun(r+1,c+1);
    }
    else{
        ll p1=(grid[r][c]-'a')+fun(r+1,c);
        ll p2=(grid[r][c]-'a')+fun(r+1,c+1);
...
void path_print(ll r,ll c){
    cout<<grid[r][c];
    if(dir[r][c]==1){
        path_print(r+1,c);
    }
    else if(dir[r][c]==2) path_print(r+1,c+1);
}
