...
ll r[mxn],c[mxn];
vector<ll>V[mxn],tmp[mxn];
ll n,m;

void rowchange(ll r1,ll r2){
    for(ll i=0;i<m;i++){
        swap(tmp[r1][i],tmp[r2][i]);
    }
}

void colchange(ll c1,ll c2){
    for(ll i=0;i<n;i++){
...
ll fun(){
    ...
    for(ll i=0;i<n-1;i++){
        for(ll j=0;j<m-1;j++){
            tot+=tmp[i][j]+tmp[i+1][j]+tmp[i][j+1]+tmp[i+1][j+1];
        }
    }
