...
ll fun(ll i,ll j,ll k){ cout<<"calling "<<i<<" "<<j<<" "<<k<<"\n";
    ll f=-1;
    if(M[0]>0){
        for(ll z=1;z<=9;z++){
            if(M[z]>0){
    ...
    for(i=9;i>=0;i--){
        ...
        for(j=9;j>=0;j--){
            ...
            for(k=9;k>=0;k--){
                ...
                V.push_back(j);
                V.push_back(k);
                sort(V.begin(),V.end());//cout<<V[0]<<" "<<V[1]<<" "<<V[2]<<"\n";

                for(ll pp=0;pp<6;pp++){
                    ll x=(V[0]*100)+(V[1]*10)+(V[2]); //cout<<"now x="<<x<<"\n";
                    if(x%8==0){ //cout<<V[0]<<" "<<V[1]<<" "<<V[2]<<"\n";
                        fun(V[0],V[1],V[2]);
                        if(xxx==1){
                            return 0;
                        }
                    ...
                    next_permutation(V.begin(),V.end()); //cout<<"perm: ";cout<<V[0]<<" "<<V[1]<<" "<<V[2]<<"\n";
                }
