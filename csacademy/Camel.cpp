...
ll dx[8] = {0, 1+1, 1+2, 1+1, 0, -1-1, -1-2, -1-1};
ll dy[8] = {1+2, 1+1, 0, -1-1, -1-2, -1-1, 0, 1+1}; ///8 Direction
#define mxn 20
...
void fun(ll x,ll y,ll mark){ //printf("x=%lld y=%lld mark=%lld\n",x,y,mark); printgrid(); //system("pause");
    if(x==1 && y==1 && mark==((n*n)+1)){ //printf("breaking condition...\n");
        f=1;
        grid[1][1]=1;
        for(ll i=1;i<=n;i++){
            for(ll j=1;j<=n;j++){
    ...
    for(ll i=0LL;i<8LL;i++){
        ll vx=x+dx[i];
        ll vy=y+dy[i]; //printf("vx=%lld vy=%lld\n",vx,vy);
        if(valid(vx,vy)){
            if(grid[vx][vy]==0 || grid[vx][vy]==1){
                grid[vx][vy]=mark+1;
                fun(vx,vy,mark+1);
                grid[vx][vy]=0;
            }
