...
map<ll,ll>dp[mxn],path[mxn];
ll n,m;

ll fun(ll p,ll s){
    if(p>m) return 0;

    if(dp[p].find(s)!=dp[p].end()) return dp[p][s];
    ...
    printf("p=%lld s=%lld add=%lld\n",p,s,add);
    p1=b[p][4]-(s+add)+fun(p+1,s+add);


    printf("p=%lld p1=%lld p2=%lld\n",p,p1,p2);
    return dp[p][s]=max(p1,p2);
}
