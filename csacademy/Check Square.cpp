    ...
    for(t=1;t<=T;t++){
        ...
        for(i=1;i<=4;i++){
            for(j=i+1;j<=4;j++){
                ...
                V[i][j].x=i;
                V[i][j].y=j;
                V[i][j].dis=sqrt(sqr(a[i][1]-a[j][1])+sqr(a[i][2]-a[j][2]));

                V[j][i].x=j;
                V[j][i].y=i;
                V[j][i].dis=sqrt(sqr(a[i][1]-a[j][1])+sqr(a[i][2]-a[j][2]));
            }
        }

        ll f=0;
        for(i=1;i<=4;i++){//printf("i=%lld\n",i);
                if(i==1) continue;
            for(j=1;j<=4;j++){ //printf("j=%lld\n",j);
                    if(fun(1,i,2,j)) continue;
                for(ll k=1;k<=4;k++){ //printf("k=%lld\n",k);
                        if(fun(1,i,3,k) || fun(3,k,2,j) || k==3) continue;
                    for(ll l=1;l<=4;l++){ //printf("l=%lld\n",l);
                        if(fun(1,i,4,l) || fun(4,l,2,j) || fun(3,k,4,l) || l==4) continue;

                        double dd=V[1][i].dis+V[2][j].dis+V[3][k].dis+V[4][l].dis;
                        double gg=4.0*V[1][i].dis; //printf("%f %f\n",dd,gg);

                        if(dd-gg<=ERR && gg-dd<=ERR){ //printf("%lld %lld %lld %lld %f %f\n",i,j,k,l,dd,gg);
                            f=1;
                        }
                    }
                }
