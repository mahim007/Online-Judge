...
int main()
{
    ...
    for(char ch='a';ch<='z';ch++){
        ...
        for(ll l=1;l<=26;l++){
            ...
            char y=s[CC[x-'a'][0]+l-1];
            //printf("x=%c y=%c\n",x,y);

            for(i=0;i<CC[x-'a'].size();i++){
                if(CC[x-'a'][i]+l-1>=sz){
                    f=0; //printf("break one.\n");
                    break;
                }

                if(s[CC[x-'a'][i]+l-1]!=y){
                    f=0; //printf("break two.\n");
                    //printf("wrong = %c  at=%lld\n",s[CC[x-'a'][i]+l-1],CC[x-'a'][i]+l-1);
                    break;
                }
            }

            if(f==0) break;
            string g=s.substr(CC[x-'a'][0],l);
            if(g.size()>mini.size()){
                mini=g;
            }
            else if(g.size()==mini.size()){
                mini=min(mini,g);
            } //cout<<"here:"<<mini<<"\n";
        }
    }
