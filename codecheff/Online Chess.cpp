...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        for(i=1;i<=n;i++){
            data D;
            cin>>D.r>>D.mn>>D.mx>>D.tym>>D.rated>>D.col;
            V.push_back(D);
        }

        mem(st,-1);
        for(i=0;i<V.size();i++){ //cout<<"now:"<<i<<"\n";
            ll f=0;
            for(j=0;j<i;j++){
                if(st[j]!=-1) continue;
                if((V[i].r>=V[j].mn && V[i].r<=V[j].mx)  && (V[j].r>=V[i].mn && V[j].r<=V[i].mx)  && (V[i].rated==V[j].rated) && ((V[i].col=="random" && V[j].col=="random")||(V[i].col=="black" && V[j].col=="white") || (V[i].col=="white" && V[j].col=="black"))){
                    st[i]=j+1;
                    st[j]=-2; //cout<<"match with "<<j<<"\n";
                    break;
                }
            }
        }
