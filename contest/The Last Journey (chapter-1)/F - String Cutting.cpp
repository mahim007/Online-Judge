...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        cin>>s;
        ll sz=s.size(); //printf("sz=%lld\n",sz);

        V.push_back(-1LL);
        V.push_back(sz-1);

        for(i=1;i<=q;i++){ //printf("i=%lld\n",i);
            ll d=arr[i];
            d--;
            ll u;
            ll v;
            ll pos=lower_bound(V.begin(),V.end(),d)-V.begin(); //printf("d=%lld pos=%lld\n",d,pos);
            if(V[pos]==d){
                u=V[pos-1]+1;
                v=V[pos+1];
            }
            ...
            for(auto it=S1.begin();it!=S1.end();it++){
                char ch=*it; //printf("ch=%c\n",*it);
                if(S2.count(ch)==1){
                    cnt++;
                }
            } //printf("out...\n");

            ans+=S1.size()+S2.size()-(2*cnt);
            S1.clear();
            S2.clear();
