...
void dijkstra(ll src){
    ...
    while(!PQ.empty()){
        ...
        for(ll i=0;i<V[u].size();i++){
            ll v=V[u][i];
            ll y=busyness[v];
            if((dis[u]+((y-x)*(y-x)*(y-x)))<dis[v]){
                dis[v]=dis[u]+((y-x)*(y-x)*(y-x));
                data Q;
                Q.node=v;
                Q.cost=dis[v];
                PQ.push(Q);
                cnt[v]++;
                if(cnt[v]>(n+mxn)) return;
            }
        }
    }
}
