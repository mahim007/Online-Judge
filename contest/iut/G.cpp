...
ll call(){
    tag[0]=tag[1]=1;
    for(ll i=4;i<=mxn;i=i+2) tag[i]=1;
    for(ll i=3;i*i<=mxn;i=i+2){
        if(tag[i]==0){
            for(ll j=i*i;j<=mxn;j=j+i+i) tag[j]=1;
        }
    }
...
int main()
{
    ...
    ll i,j,k;
    ll n,m,q;
    call(); //cout<<"size="<<V.size()<<"\n";
    //for(i=0;i<20;i++) printf("%lld ",V[i]);printf("\n");

    cin>>n;
    k=0;
    ll r=1;
    ll c=0;
    ll cnt=n*n; //cout<<"cnt:"<<cnt<<"\n";
    while(cnt>0){
        for(i=c+1;i<=n;i++){ //printf("i1=%lld ",i);
            if(cnt<=0) break;
            if(i>n) break;
            if(grid[r][i]!=0) break;
            grid[r][i]=V[k++];
        ...
        for(i=r+1;i<=n;i++){ //printf("i1=%lld ",i);
            if(cnt<=0) break;
            if(i>n) break;
            if(grid[i][c]!=0) break;
            grid[i][c]=V[k++];
        ...
        for(i=c-1;i>=1;i--){ //printf("i1=%lld ",i);
            if(cnt<=0) break;
            if(i<1) break;
            if(grid[r][i]!=0) break;
            grid[r][i]=V[k++];
        ...
        for(i=r-1;i>=1;i--){ //printf("i1=%lld ",i);
            if(cnt<=0) break;
            if(i<1) break;
            if(grid[i][c]!=0) break;
            grid[i][c]=V[k++];
