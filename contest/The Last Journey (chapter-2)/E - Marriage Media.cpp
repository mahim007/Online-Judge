...
#define mxn 59
ll boy[mxn],girl[mxn],vis[mxn];
ll n,m;

struct data{
    ll id,age,height,divorce;
};
...
ll fun(ll x,ll y){
    if((ll)abs(B[x].height-G[y].height)>12LL) return 0;
    if((ll)abs(B[x].age-G[y].age)>5LL) return 0;
    if(B[x].divorce!=G[y].divorce) return 0;
    return 1;
}
