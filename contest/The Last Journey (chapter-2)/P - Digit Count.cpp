...
ll fun(ll pos,ll dig,ll last){ //printf("pos=%lld dig=%lld last=%lld num=%lld\n",pos,dig,last,num); //system("pause");
    if(dig<1LL) return 1LL;
    if(pos==0LL) return 0LL;


    if(dp[dig][last]!=-1) return dp[dig][last];
    ll res=0;
    for(ll i=m;i>=1LL;i--){//printf("i=%lld\n",i);
        ll dif=abs(last-a[i]); //printf("dif=%lld\n",dif);
        if(dif<=2LL) res+=fun(i,dig-1LL,a[i]);
    }

//    printf("pos:%lld finished...\n",pos);
//    res+=fun(pos-1,n-1,a[pos-1LL],a[pos-1LL]);
    return dp[dig][last]=res;
}
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        for(i=1;i<=m;i++) scanLLD(a[i]);
        //printf("pos=%lld dig=%lld last=%lld\n",m,n-1,a[m]);

        ll ans=0;
        mem(dp,-1LL);
        for(i=m;i>=1;i--){ //printf("calling:%lld\n",i);
            ans+=fun(i,n-1,a[i]);
        }
        printf("Case %lld: %lld\n",t,ans);
    }
