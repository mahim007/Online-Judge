...
ll process(Matrix &m){
    ...
    for(ll i=1;i<=4;i++) ans.mat[i][i]=1;
    ...
    ll sum=0; //printf("%lld %lld %lld %lld\n",ans.mat[1][1],ans.mat[1][2],ans.mat[1][3],ans.mat[1][4]);
    sum=(ans.mat[1][4]*c);//+(ans.mat[1][2]*1)+(ans.mat[1][3]*1)+(ans.mat[1][4]*1);
    sum%=mod;
    return sum;
}
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        Matrix m;
        init(m);
//        ll res=process(m); //printf("res=%lld\n",res);
//        ll d=res;
//        ll cnt=0;
//        ll p=0;
//        while(d){
