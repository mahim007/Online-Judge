...
ll dijkstra(){
    ...
    while(!pq.empty()){
        data U=pq.top(); //printf("now x=%lld y=%lld z=%lld\n",U.x,U.y,U.z);
        pq.pop();

        if(U.x==n && U.y==2) return U.z;

        for(ll i=0;i<4;i++){
            ll vx=U.x+dx[i];
            ll vy=U.y+dy[i];
            if(valid(vx,vy)){
                if(U.z+grid[vx][vy]<cost[vx][vy]){
                    cost[vx][vy]=U.z+grid[vx][vy];
...
int main()
{
    ...
    while(scanf("%lld",&n)==1){
        ...
        for(i=1;i<=n;i++){
            scanf("%lld %lld %lld",&grid[i][1],&grid[i][2],&grid[i][3]);
