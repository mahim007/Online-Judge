...
void call(ll l){ //cout<<"l="<<l<<"\n";
    if(V.size()==ans){ //for(ll i=0;i<V.size();i++) cout<<V[i]<<" "; cout<<"\n";
        cnt++;
        return;
    }

    for(ll i=l+1;i<digit.size();i++){
        if(used[i]==0){

            ll sz=V.size();
            sz--;
            if(sz>=0){
                if(digit[i]<=V[sz]) continue;
            }
