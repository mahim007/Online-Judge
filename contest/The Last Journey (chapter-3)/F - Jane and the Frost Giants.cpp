...
ll bfs(ll jx,ll jy){ //printf("%lld %lld %lld %lld\n",jx,jy,fx,fy);
    data D;
    queue<data>Q;
    ...
    while(!Q.empty()){
        ...
        ll ux=D.x;
        ll uy=D.y;
        char ch=D.ch; //printf("now turn=%c %lld %lld\n",ch,ux,uy); //system("pause");
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        ll ans=bfs(src.first,src.second);
        cout<<"Case "<<t<<": ";
        if(ans==-1){
            cout<<"IMPOSSIBLE\n";
        }
