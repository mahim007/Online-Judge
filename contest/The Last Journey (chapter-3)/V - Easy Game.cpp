...
ll fun(ll l,ll r){
    if(l>r){ //printf("p=%lld l=%lld r=%lld lsum=%lld rsum=%lld res=%lld\n",p,l,r,lsum,rsum,abs(lsum-rsum));
        return 0LL;
    }
    ...
    for(ll i=l;i<=r;i++){
        s+=a[i];
        res=max(res,s-fun(i+1,r));
    }

    s=0;
    for(ll i=r;i>=l;i--){
        s+=a[i];
        res=max(res,s-fun(l,i-1));
    }
