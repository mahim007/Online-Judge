...
void dfs1(ll u,ll pos,ll par){ //printf("u=%lld pos=%lld par=%lld\n",u,pos,par);
    if(vis[u]!=0){
        if((pos-vis[u])%2==1){
            oddcycle=max(oddcycle,pos-vis[u]);
        }
        return;
    }
