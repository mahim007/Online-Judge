...
bool is_collinear(ll i,ll j,ll k){ //printf("i=%lld j=%lld k=%lld\n",i,j,k);
    ll ans=x[i]*(y[j]-y[k])+x[j]*(y[k]-y[i])+x[k]*(y[i]-y[j]); //printf("ans=%lld\n",ans);
    return ans==0LL;
}

ll fun(ll mask){ //printf("mask=%lld\n",mask);
    if(mask==((1<<n)-1)) return 0;

    ll &res=dp[mask];
    if(dp[mask]!=-1) return dp[mask];

    if(__builtin_popcount(mask)==(n-1)) return dp[mask]=1LL;

    res=inf;
    ll i;

    for(i=0;check(mask,i);i++);

    if(1){
        for(ll j=0;j<n;j++){
            if(check(mask,j)==1LL || i==j) continue;
            ll tmp=biton(mask,i);
            tmp=biton(tmp,j);
            res=min(res,1+fun(tmp|cl[i][j]));
        }
    }
...
{ //freopen("brushout.txt","w",stdout);
    ...
    for(t=1;t<=T;t++){
        ...
        for(i=0;i<n;i++){
            for(j=i+1;j<n;j++){
                for(k=0;k<n;k++){
                    if(i!=k && j!=k && is_collinear(i,j,k)){
                        cl[i][j]|=(1LL<<k);
                        cl[j][i]|=(1LL<<k); //printf("cl[i][j]=%lld\n",cl[i][j]);
                    }
                }
            }
        }
