...
ll valid(ll x,ll y,char des){
    if(x<1 || x>n) return 0;
    if(y<1 || y>n) return 0;
    if(grid[x][y]=='#') return 0;
    if(grid[x][y]==des) return 1;
    if(grid[x][y]>='A' && grid[x][y]<='Z') return 0;
    return 1;
}

ll bfs(char src,char des,ll x,ll y){ //cout<<src<<" "<<des<<" "<<x<<" "<<y<<"\n";
    vis[x][y]=1;
    queue<pair<ll,ll> >Q;
    Q.push({x,y});
    ...
    while(!Q.empty()){
        ...
        if(grid[ux][uy]==des) return vis[ux][uy]-1;

        for(ll i=0;i<4;i++){
            ll vx=ux+dx[i];
            ll vy=uy+dy[i];

            if(valid(vx,vy,des)==1 && vis[vx][vy]==0){
...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        for(char a='A';a<q;a++){
            mem(vis,0);
            ll d=bfs(a,a+1,D[a-'A'].x,D[a-'A'].y);
            if(d==-1){
                cout<<"Impossible\n";
                f=1;
                break;
