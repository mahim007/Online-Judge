...
void fun0(ll x,ll y){ //cout<<x<<" "<<y<<"\n";
    for(ll i=1;i<=4;i++){
        for(ll j=1;j<=4;j++){
            if(g[i][j]==0) continue;

            ll vx=i+x; //cout<<"\n";
            ll vy=j+y; //cout<<vx<<" "<<vy<<" ";
            if(valid(vx,vy)==0) continue;
            //if(g[vx][vy]!=0 && g[x][y]!=g[vx][vy]) continue;


            if(g[vx][vy]==0){ //cout<<"move";
                g[vx][vy]=g[i][j];
                g[i][j]=0;
            }
            else if(g[i][j]==g[vx][vy] && mix[vx][vy]==0 && mix[i][j]==0){ //cout<<"merge";
                mix[vx][vy]=1;
                g[vx][vy]+=g[i][j];
                g[i][j]=0;
            }
