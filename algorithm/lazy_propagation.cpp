...
void build(ll node,ll l,ll r){ //printf("node=%lld l=%lld r=%lld\n",node,l,r);
    lazy[node]=0;
    if(l==r){
        tree[node].one=0;
        tree[node].two=0;
...
ll query(ll node,ll l,ll r,ll x,ll y){
    ...
    if(lazy[node]!=0){//printf("updating...\n");
        ll i=0;
        while(i<lazy[node]){
            swap(tree[node].zero,tree[node].two);
            swap(tree[node].one,tree[node].two);
    ...
    if(l>=x && r<=y) return tree[node].zero;

    ll mid=(l+r)/2;
    ll p1=query(node*2,l,mid,x,y);
    ll p2=query((node*2)+1,mid+1,r,x,y);
    return p1+p2;
}
