    ...
    Matrix operator * (const Matrix& m) const{
        ...
        for(ll i=1;i<=n;i++){
            for(ll j=1;j<=n;j++){
                tmp.mat[i][j]=inf;
                for(ll k=1;k<=n;k++){
                    tmp.mat[i][j]=min(tmp.mat[i][j],mat[i][k]+m.mat[k][j]);
                }
            }
        }
...
//    }
    ...
    Matrix ans=calc(p,q-1);

    for(i=1;i<=n;i++){
        for(j=1;j<=n;j++){
            if(ans.mat[i][j]>=inf) cout<<"inf";
            else cout<<ans.mat[i][j];
            cout<<"\t";
        }
