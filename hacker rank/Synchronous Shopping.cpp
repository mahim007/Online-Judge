    ...
    for(i=1;i<=n;i++){
        ...
        while(!S.empty()){
            ...
            for(i=0;i<graph[u].size();i++){
                ...
                ll v_cost=cost[u][i];
                ll v_mask=msk[v];
                ll tot_mask=u_mask|v_mask;

                if((u_cost+v_cost) < dis[v][tot_mask]){
                    S.erase({dis[v][tot_mask], {v,tot_mask} });
                    dis[v][tot_mask]=u_cost+v_cost;
                    S.insert({u_cost+v_cost, {v,tot_mask} });
                }
            }
        }
