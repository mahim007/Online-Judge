...
void precalc(){
    for(ll i=1;i<=32;i++){
        for(ll j=0;j<5;j++){
            if(check(i,j)){
                res[i].insert(res[i].end(),a[j+1].begin(),a[j+1].end());
            }
        }
    }

    for(ll i=1;i<=32;i++) sort(res[i].begin(),res[i].end());
}
...
int main()
{
    ...
    cin>>n;
    for(i=1;i<=n;i++) cin>>cam[i];
    for(i=1;i<=n;i++) cin>>price[i];
    for(i=1;i<=n;i++) a[cam[i]].push_back(price[i]);
    //for(i=1;i<=5;i++) sort(a[i].begin(),a[i].end());
    ...
    for(k=1;k<=q;k++){
        ...
        if(cat>res[mask].size()) cout<<"-1\n";
        else cout<<res[mask][cat-1]<<"\n";
    }
