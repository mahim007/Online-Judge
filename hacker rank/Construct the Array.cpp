...
int main()
{
    ...
    for(i=0;i<mxn;i=i+2){
        same[i]=(diff[i]+1);
        same[i]=(same[i]+mod)%mod;
        same[i+1]=diff[i+1]-1;
        same[i+1]=(same[i+1]+mod)%mod;
    }
    ...
    if(m==1){
        ans=(ans-((q-1)*diff[n-3]))%mod;
    }
    else{
        ans=(ans-((q-2)*diff[n-3])-same[n-3])%mod;
    }
    cout<<(ans+mod)%mod<<"\n";

//    for(i=0;i<10;i++) cout<<diff[i]<<" ";cout<<"\n";
//    for(i=0;i<10;i++) cout<<same[i]<<" ";cout<<"\n";
