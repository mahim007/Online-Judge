...
ll query(ll n,ll l,ll r,ll x,ll y){
    ...
    ll mid=(l+r)/2;
    ll p1=query(2*n,l,mid,x,y);
    ll p2=query((2*n)+1,mid+1,r,x,y);
    return p1+p2;
}
...
//    for(i=1;i<=n;i++) cout<<mxi[i]<<" ";cout<<"\n";
    ...
    for(i=n;i>=1;i--){
        update(1,1,n,i,mxi[i]);

        while(V.size() && V.back().l==i){
            ans[V.back().id]=query(1,1,n,V.back().l,V.back().r);
            V.pop_back();
        }
    }
