...
int main()
{
    ...
    for(t=1;t<=T;t++){
        ...
        ll root=floor(bin(b))-floor(bin(a-1)); //printf("root=%lld\n",root);
        ll qbrt=floor(cube(b))-floor(cube(a-1)); //printf("cube=%lld\n",qbrt);
        ll ans=(b-a+1)-root-qbrt;
        ll add=fun(b)-fun(a-1); //printf("%lld %lld add=%lld\n",fun(b),fun(a-1),add);
        ans+=add;
        //if(a==1) ans++;
        printf("%lld\n",ans);
    }
